...
class ScreenshotHandler(QObject):
    ...
    def _hide_windows(self) -> None:
        ...
        logger.info("[ScreenshotHandler] 开始隐藏窗口")

        # 记录原始显示状态
        self._ball_was_visible = self._floating_ball.isVisible()
        ...
        try:
            ...
            logger.info(
                "[ScreenshotHandler] 隐藏完成: ball_visible=%s chat_visible=%s",
                self._ball_was_visible,
                self._chat_window_was_visible,
            )
        except Exception:
            logger.exception("[ScreenshotHandler] 隐藏窗口失败")
    ...
    def _restore_windows(self) -> None:
        ...
        logger.info("[ScreenshotHandler] 开始恢复窗口")

        try:
            if self._ball_pos is not None:
                self._floating_ball.move(self._ball_pos)

            if self._chat_pos is not None:
                self._floating_ball._compact_window.move(self._chat_pos)

            # 先恢复悬浮球本体
            self._floating_ball.setWindowOpacity(1.0)
            self._floating_ball.show()
            self._floating_ball.raise_()

            # 聊天窗口按原状态恢复
            if getattr(self, "_chat_window_was_visible", False):
                self._floating_ball._compact_window.setWindowOpacity(1.0)
                self._floating_ball._compact_window.show()
                self._floating_ball._compact_window.raise_()
            else:
                self._floating_ball._compact_window.hide()

            QApplication.processEvents()
            logger.info("[ScreenshotHandler] 窗口恢复完成")
        except Exception:
            logger.exception("[ScreenshotHandler] 恢复窗口失败")
